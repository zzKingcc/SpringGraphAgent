package com.zxc.stringer.agent.orchestration;

import com.zxc.stringer.agent.tools.ToolRouter;
import com.zxc.stringer.common.util.TokenUsageTracker;
import com.zxc.stringer.agent.cancellation.CancellationTracker;
import com.zxc.stringer.infrastructure.memory.DualConstraintChatMemory;
...
public class AgentOrchestrationService {
    ...
    /** 与 RedisCheckpointSaver 共享的图状态序列化器(已注册 ChatMessage/ToolExecutionRequest 序列化器) */
    ...
    public AgentOrchestrationService(
        ...
        // 从 ToolRouter 获取工具规格和授权标记(反射扫描已在 ToolRouter 构造函数完成)
        this.toolSpecs = toolRouter.getToolSpecifications();
        this.toolsRequiringApproval = toolRouter.getToolsRequiringApproval();
    ...
    private Map<String, Object> toolsNode(MessagesState<ChatMessage> state) {
        ...
        for (ToolExecutionRequest request : aiMessage.toolExecutionRequests()) {
            ...
            // 通过 ToolRouter 统一路由执行(内含 Token 用量统计)
            String result = toolRouter.execute(request);
