package com.zxc.stringer.infrastructure.checkpoint;

import com.zxc.stringer.common.exception.ChatMemoryException;
 ...
 *
 * <p>序列化方式:通过 CheckpointListSerializer 委托给与 graph 共享的 StateSerializer
 * (ObjectStreamStateSerializer,已注册 ChatMessage / ToolExecutionRequest 自定义序列化器),
 * 将 Checkpoint.state 中的 LangChain4j 对象正确序列化/反序列化后,Base64 编码落 Redis。
 * 注意:不能直接用 Java 原生 ObjectOutputStream 序列化 Checkpoint 本身,
 * Checkpoint 及 ChatMessage / ToolExecutionRequest 均未实现 java.io.Serializable,会抛 NotSerializableException。</p>
 */
